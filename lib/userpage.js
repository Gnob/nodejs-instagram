...
var download_user = function(username,dir,filter) {
    ...
    return scan_user(
        ...
        function(data) {
            for (i in data.nodes) {
                console.log(data.nodes[i].display_src);
...
var crawl_image_url = function(username,cb) {
  return scan_user(
        username,
        function(data) {
            for (i in data.nodes) {
                cb(username, data.nodes[i].id, data.nodes[i].display_src);
            }
        }
    );
}
...
module.exports = {
    ...
    crawl_image_url : crawl_image_url,
}
